...
function render() {
  ...
  const maxChars = Math.max(
    18,
    Math.floor((printWidth - margin * 2) / charWidth),
  );
  const dividerLine = "=".repeat(maxChars);
  const subDividerLine = "-".repeat(maxChars);

  const usablePhotoWidth = printWidth - margin * 2;
  const photoHeight = loadedImage
    ? Math.round((loadedImage.height / loadedImage.width) * usablePhotoWidth)
    : 0;
  ...
  // Helper to render photo inline with text flow
  ...
  pCtx.fillText(
    `[ ${elements.headerSub.value.toUpperCase()} ]`,
    printWidth / 2,
    curY,
  );
  curY += lineHeight;
  ...
  pCtx.textAlign = "left";
  pCtx.fillText(
    `DATE: 2026-09-02      REC #: ${Math.floor(1000 + Math.random() * 9000)}`,
    margin,
    curY,
  );
  curY += lineHeight;
  ...
  for (let y = 0; y < paperHeight; y++) {
    ...
    for (let x = 0; x < printWidth; x++) {
      ...
      if (showTear) {
        const toothSize = 12;
        const toothDepth = 6;
        const toothY = Math.floor(
          Math.abs(Math.sin((x / toothSize) * Math.PI)) * toothDepth,
        );
  ...
  if (aspect === "native") {
    ...
    const fitScale = Math.min(
      maxDisplayW / printWidth,
      maxDisplayH / paperHeight,
      2.0,
    );
...
[
  ...
  elements.transparentBgInput,
  elements.imagePosInput,
  elements.headerTitle,
  elements.headerSub,
  ...elements.trackInputs,
].forEach((input) => {
